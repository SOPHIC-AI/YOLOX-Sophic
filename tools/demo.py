...
def image_demo(predictor, vis_folder, path, current_time, save_result):
    ...
    for image_name in files:
        with open('new/like.txt', 'a') as f:
            f.write(image_name + ' ' + 'like' + ' ')
            ...
            if outputs[0] != None:
                for i in range(len(outputs[0])):
                    f.write(str(int(int(outputs[0][i][0])/ratio)) + ' ' + str(int(int(outputs[0][i][1])/ratio)) + ' ' + str(int(int(outputs[0][i][2])/ratio)) + ' ' + str(int(int(outputs[0][i][3])/ratio)) + ' ')
            f.write('\n')
            result_image = predictor.visual(outputs[0], img_info, predictor.confthre)
            # for i in range(len(bboxes)):
            #     f.write(str(int(bboxes[i][0])) + ' ' + str(int(bboxes[i][1])) + ' ' + str(int(bboxes[i][2])) + ' ' + str(int(bboxes[i][3])) + ' ')
            # f.write('\n')
            if save_result:
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    vid_writer = cv2.VideoWriter(
        'output.avi', cv2.VideoWriter_fourcc(*"mp4v"), fps, (int(width), int(height))
    )
    while True:
